...
async function patchDocument(response, request){
  ...
  if(url.hostname.endsWith("github.io") && !text.includes("basa-cloud-bootstrap")){
    const bootstrap=`<script id="basa-cloud-bootstrap">(function(){function load(){if(window.__basaCloudLoaded)return;if(typeof window.login==='function'&&typeof window.save==='function'&&typeof window.teacher==='function'){window.__basaCloudLoaded=true;var s=document.createElement('script');s.id='basa-cloud-sync-script';s.src='/Basa-Track/cloud-sync.js';s.onload=function(){var p=document.createElement('script');p.id='basa-teacher-preview-script';p.src='/Basa-Track/teacher-preview.js';p.onload=function(){try{var q=new URL(location.href).searchParams;if(q.get('learner')==='1'){document.querySelectorAll('#home button').forEach(function(b,i){if(i===1||/Teacher Dashboard/i.test(b.textContent||''))b.style.display='none'});['teacher','teacherLogin','pupilManager','rosterEditor','teacherBackBtn'].forEach(function(id){var e=document.getElementById(id);if(e)e.style.display='none'});if(typeof window.show==='function')window.show('login');}else if(q.get('teacher')==='1'){var hasTeacher=sessionStorage.getItem('basa_cloud_role')==='teacher'||localStorage.getItem('basa_teacher_token');if(hasTeacher){if(typeof window.show==='function')window.show('teacher')}else if(typeof window.basaCloudTeacherGate==='function'){window.basaCloudTeacherGate();}}}catch(e){console.warn(e)}};document.body.appendChild(p)};document.body.appendChild(s);}else{setTimeout(load,50);}}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',load)}else{load()}})();</script>`;
