...
async function patchDocument(response, request){
  ...
  if(url.hostname.endsWith("github.io") && !text.includes("basa-cloud-bootstrap")){
    const bootstrap=`<script id="basa-cloud-bootstrap">(function(){function load(){if(window.__basaCloudLoaded)return;if(typeof window.login==='function'&&typeof window.save==='function'&&typeof window.teacher==='function'){window.__basaCloudLoaded=true;var s=document.createElement('script');s.id='basa-cloud-sync-script';s.src='/Basa-Track/cloud-sync.js?v=21';s.onload=function(){var p=document.createElement('script');p.id='basa-teacher-preview-script';p.src='/Basa-Track/teacher-preview.js?v=21';p.onload=function(){function route(){try{var q=new URL(location.href).searchParams;if(q.get('learner')==='1'){document.querySelectorAll('#home button').forEach(function(b,i){if(i===1||/Teacher Dashboard/i.test(b.textContent||''))b.style.display='none'});['teacher','teacherLogin','pupilManager','rosterEditor','teacherBackBtn'].forEach(function(id){var e=document.getElementById(id);if(e)e.style.display='none'});if(typeof window.show==='function')window.show('login');}else if(q.get('teacher')==='1'){var tok=sessionStorage.getItem('basa_cloud_token')||sessionStorage.getItem('basa_teacher_token')||localStorage.getItem('basa_teacher_token');var role=sessionStorage.getItem('basa_cloud_role')||localStorage.getItem('basa_teacher_role');var hasTeacher=!!tok&&role==='teacher';if(hasTeacher){if(typeof window.show==='function')window.show('teacher')}else{sessionStorage.removeItem('basa_cloud_token');sessionStorage.removeItem('basa_cloud_role');sessionStorage.removeItem('basa_teacher_token');sessionStorage.removeItem('basa_teacher_role');localStorage.removeItem('basa_teacher_token');localStorage.removeItem('basa_teacher_role');if(typeof window.basaCloudTeacherGate==='function'){window.basaCloudTeacherGate();}else{setTimeout(route,100);}}}}catch(e){console.warn(e)}}route();};document.body.appendChild(p)};document.body.appendChild(s);}else{setTimeout(load,50);}}if(document.readyState==='loading'){document.addEventListener('DOMContentLoaded',load)}else{load()}})();</script>`;
